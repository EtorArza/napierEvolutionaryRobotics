...
def load_all_fitnesses(filename,objective) :
    with open(filename) as file :
        ...
        for row in csv_data :
            ...
            for i in range(3,len(row[:-1]),int(row[2])) :
                if(float(row[i+objective]) > best_fitness) :
                    best_fitness = float(row[i+objective])
                    best_id = int(i/int(row[2]) - 1)
                avg_fitness += float(row[i+objective])
...
def get_best_fitness(filename, objective):
    '''
